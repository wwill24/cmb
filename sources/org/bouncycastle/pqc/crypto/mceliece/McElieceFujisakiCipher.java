...
public class McElieceFujisakiCipher implements MessageEncryptor {
    ...
    private void initCipherDecrypt(McElieceCCA2PrivateKeyParameters mcElieceCCA2PrivateKeyParameters) {
        ...
        this.f24019n = mcElieceCCA2PrivateKeyParameters.getN();
        this.f24020t = mcElieceCCA2PrivateKeyParameters.getT();
    }

    private void initCipherEncrypt(McElieceCCA2PublicKeyParameters mcElieceCCA2PublicKeyParameters) {
        this.messDigest = Utils.getDigest(mcElieceCCA2PublicKeyParameters.getDigest());
        this.f24019n = mcElieceCCA2PublicKeyParameters.getN();
        this.f24018k = mcElieceCCA2PublicKeyParameters.getK();
        this.f24020t = mcElieceCCA2PublicKeyParameters.getT();
    }
    ...
    public byte[] messageDecrypt(byte[] bArr) throws InvalidCipherTextException {
        if (!this.forEncryption) {
            int i10 = (this.f24019n + 7) >> 3;
            int length = bArr.length - i10;
            byte[][] split = ByteUtils.split(bArr, i10);
            byte[] bArr2 = split[0];
            byte[] bArr3 = split[1];
            GF2Vector[] decryptionPrimitive = McElieceCCA2Primitives.decryptionPrimitive((McElieceCCA2PrivateKeyParameters) this.key, GF2Vector.OS2VP(this.f24019n, bArr2));
            ...
            if (Conversions.encode(this.f24019n, this.f24020t, bArr5).equals(gF2Vector)) {
                return bArr4;
            }
    ...
    public byte[] messageEncrypt(byte[] bArr) {
        if (this.forEncryption) {
            GF2Vector gF2Vector = new GF2Vector(this.f24018k, this.sr);
            ...
            byte[] encoded2 = McElieceCCA2Primitives.encryptionPrimitive((McElieceCCA2PublicKeyParameters) this.key, gF2Vector, Conversions.encode(this.f24019n, this.f24020t, bArr2)).getEncoded();
            DigestRandomGenerator digestRandomGenerator = new DigestRandomGenerator(new SHA1Digest());
            digestRandomGenerator.addSeedMaterial(encoded);
