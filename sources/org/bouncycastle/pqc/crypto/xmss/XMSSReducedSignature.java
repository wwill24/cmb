...
public class XMSSReducedSignature implements XMSSStoreableObjectInterface {
            ...
            if (access$100 != null) {
                if (access$100.length == (len * treeDigestSize) + (height * treeDigestSize)) {
                    ...
                    for (int i11 = 0; i11 < len; i11++) {
                        bArr[i11] = XMSSUtil.extractBytesAtOffset(access$100, i10, treeDigestSize);
                        i10 += treeDigestSize;
                    }
                    this.wotsPlusSignature = new WOTSPlusSignature(this.params.getWOTSPlus().getParams(), bArr);
                    list = new ArrayList<>();
                    for (int i12 = 0; i12 < height; i12++) {
                        list.add(new XMSSNode(i12, XMSSUtil.extractBytesAtOffset(access$100, i10, treeDigestSize)));
                        i10 += treeDigestSize;
                    }
                } else {
    ...
    public byte[] toByteArray() {
        ...
        for (byte[] copyBytesAtOffset : byteArray) {
            XMSSUtil.copyBytesAtOffset(bArr, copyBytesAtOffset, i10);
            i10 += treeDigestSize;
        }
