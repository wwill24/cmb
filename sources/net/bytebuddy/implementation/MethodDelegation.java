...
public class MethodDelegation implements Implementation.Composable {
    ...
    protected interface ImplementationDelegate extends InstrumentedType.Prepareable {
        ...
        public static class ForConstruction implements ImplementationDelegate {
            ...
            protected static ImplementationDelegate of(TypeDescription typeDescription2, MethodList<?> methodList, MethodDelegationBinder methodDelegationBinder) {
                ...
                Iterator<E> it = methodList.iterator();
                while (it.hasNext()) {
        ...
        public static abstract class ForField implements ImplementationDelegate {
            ...
            public Compiled compile(TypeDescription typeDescription) {
                ...
                if (resolve.getType().asErasure().isVisibleTo(typeDescription)) {
                    MethodList<MethodDescription> methodList = (MethodList) this.methodGraphCompiler.compile((TypeDefinition) resolve.getType(), typeDescription).listNodes().asMethodList().filter(this.matcher);
                    ArrayList arrayList = new ArrayList(methodList.size());
                    MethodDelegationBinder of2 = TargetMethodAnnotationDrivenBinder.of(this.parameterBinders);
                    for (MethodDescription compile : methodList) {
                        arrayList.add(of2.compile(compile));
                    }
        ...
        public static class ForMethodReturn implements ImplementationDelegate {
            ...
            public Compiled compile(TypeDescription typeDescription) {
                MethodList methodList = (MethodList) new MethodList.Explicit(CompoundList.of(typeDescription.getDeclaredMethods().filter(ElementMatchers.isStatic().or(ElementMatchers.isPrivate())), (FilterableList) this.methodGraphCompiler.compile((TypeDefinition) typeDescription).listNodes().asMethodList())).filter(ElementMatchers.named(this.name).and(ElementMatchers.takesArguments(0)).and(ElementMatchers.not(ElementMatchers.returns((ElementMatcher<? super TypeDescription>) ElementMatchers.isPrimitive().or(ElementMatchers.isArray())))));
                if (methodList.size() != 1) {
                    ...
                    MethodList<MethodDescription> methodList2 = (MethodList) this.methodGraphCompiler.compile((TypeDefinition) ((MethodDescription) methodList.getOnly()).getReturnType(), typeDescription).listNodes().asMethodList().filter(this.matcher);
    ...
    public static class WithCustomProperties {
        ...
        public WithCustomProperties filter(ElementMatcher<? super MethodDescription> elementMatcher) {
            return new WithCustomProperties(this.ambiguityResolver, this.parameterBinders, this.bindingResolver, new ElementMatcher.Junction.Conjunction((ElementMatcher<? super W>[]) new ElementMatcher[]{this.matcher, elementMatcher}));
        }
        ...
        public MethodDelegation to(Object obj, String str, MethodGraph.Compiler compiler) {
            return to(obj, (Type) obj.getClass(), str, compiler);
        }
        ...
        public MethodDelegation to(Object obj, Type type, String str, MethodGraph.Compiler compiler) {
            return to(obj, (TypeDefinition) TypeDefinition.Sort.describe(type), str, compiler);
        }
