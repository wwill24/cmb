...
public class ViewProcessor {
    ...
    private ViewCache applyServerMerge(ViewCache viewCache, Path path, CompoundWrite compoundWrite, WriteTreeRef writeTreeRef, Node node, boolean z10, ChildChangeAccumulator childChangeAccumulator) {
        ...
        for (Map.Entry next : childCompoundWrites.entrySet()) {
            ...
            if (node2.hasChild(childKey)) {
                viewCache2 = applyServerOverwrite(viewCache2, new Path(childKey), ((CompoundWrite) next.getValue()).apply(node2.getImmediateChild(childKey)), writeTreeRef, node, z10, childChangeAccumulator);
            }
        }
        ViewCache viewCache3 = viewCache2;
        for (Map.Entry next2 : childCompoundWrites.entrySet()) {
            ChildKey childKey2 = (ChildKey) next2.getKey();
            CompoundWrite compoundWrite3 = (CompoundWrite) next2.getValue();
            if (viewCache.getServerCache().isCompleteForChild(childKey2) || compoundWrite3.rootWrite() != null) {
                z12 = false;
            } else {
                z12 = true;
            }
            if (!node2.hasChild(childKey2) && !z12) {
                viewCache3 = applyServerOverwrite(viewCache3, new Path(childKey2), ((CompoundWrite) next2.getValue()).apply(node2.getImmediateChild(childKey2)), writeTreeRef, node, z10, childChangeAccumulator);
            }
        }
        return viewCache3;
    }
    ...
    private ViewCache applyServerOverwrite(ViewCache viewCache, Path path, Node node, WriteTreeRef writeTreeRef, Node node2, boolean z10, ChildChangeAccumulator childChangeAccumulator) {
        ...
        if (path.isEmpty()) {
            ...
            ViewCache updateServerSnap = viewCache2.updateServerSnap(indexedNode, z11, nodeFilter.filtersNodes());
        ...
        Path path2 = path;
        z11 = false;
        ViewCache updateServerSnap2 = viewCache2.updateServerSnap(indexedNode, z11, nodeFilter.filtersNodes());
    ...
    private ViewCache generateEventCacheAfterServerEvent(ViewCache viewCache, Path path, WriteTreeRef writeTreeRef, NodeFilter.CompleteChildSource completeChildSource, ChildChangeAccumulator childChangeAccumulator) {
        ...
        return viewCache2.updateEventSnap(indexedNode, z11, this.filter.filtersNodes());
    }
    ...
    public ProcessorResult applyOperation(ViewCache viewCache, Operation operation, WriteTreeRef writeTreeRef, Node node) {
        ...
        int i10 = AnonymousClass2.$SwitchMap$com$google$firebase$database$core$operation$Operation$OperationType[operation.getType().ordinal()];
        if (i10 == 1) {
            ...
            if (overwrite.getSource().isFromUser()) {
                ...
                viewCache2 = applyServerOverwrite(viewCache, overwrite.getPath(), overwrite.getSnapshot(), writeTreeRef, node, z10, childChangeAccumulator);
            }
        } else if (i10 == 2) {
            ...
            if (merge.getSource().isFromUser()) {
                ...
                viewCache2 = applyServerMerge(viewCache, merge.getPath(), merge.getChildren(), writeTreeRef, node, z11, childChangeAccumulator);
            }
        } else if (i10 == 3) {
